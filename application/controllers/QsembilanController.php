...
class QsembilanController extends CI_Controller{
  public function __construct()
  {
    parent::__construct();
    $model = array('QsembilanModel','PasienModel');
  ...
  public function index()
  {
      ...
      $data['data'] = $this->QsembilanModel->lihat_satu_pasien($id);
      $data['nilai'] = $this->QsembilanModel->lihat_satu_nilai($ciri);
      $data['title'] = '9. Gejala Kardiovaskuler';
    
    $this->load->view('templates/header_tes',$data);
    $this->load->view('frontend/q9/index',$data);
  ...
  public function update()
  {
    if (isset($_POST['update'])){
      ...
      $data = array(
        ...
        'nilai_total' => $item_satu + $item_dua + $item_tiga + $item_empat + $item_lima + $item_enam + $item_tujuh + $item_delapan + $item_sembilan
      );
      $save = $this->QsembilanModel->update_qsembilan($id,$data);
      if ($save>0){

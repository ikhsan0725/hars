...
class QsebelasController extends CI_Controller{
  public function __construct()
  {
    parent::__construct();
    $model = array('QsebelasModel','PasienModel');
  ...
  public function index()
  {
        ...
        $data['data'] = $this->QsebelasModel->lihat_satu_pasien($id);
        $data['nilai'] = $this->QsebelasModel->lihat_satu_nilai($ciri);
        $data['title'] = '11. Gejala <i>Astrointestinal</i>';
    
    $this->load->view('templates/header_tes',$data);
    $this->load->view('frontend/q11/index',$data);
  ...
  public function update()
  {
    if (isset($_POST['update'])){
      ...
      $data = array(
        ...
        'nilai_total' => $item_satu + $item_dua + $item_tiga + $item_empat + $item_lima + $item_enam + $item_tujuh + $item_delapan + $item_sembilan + $item_sepuluh + $item_sebelas
      );
      $save = $this->QsebelasModel->update_qsebelas($id,$data);
      if ($save>0){

...
class QempatbelasController extends CI_Controller{
  public function __construct()
  {
    parent::__construct();
    $model = array('QempatbelasModel','PasienModel');
  ...
  public function index()
  {
        ...
        $data['data'] = $this->QempatbelasModel->lihat_satu_pasien($id);
        $data['nilai'] = $this->QempatbelasModel->lihat_satu_nilai($ciri);
        $data['title'] = '14. Perilaku Sewaktu Wawancara';
    
    $this->load->view('templates/header_tes',$data);
    $this->load->view('frontend/q14/index',$data);
  ...
  public function update()
  {
    if (isset($_POST['update'])){
      ...
      $item_empatbelas = $this->input->post('item_empatbelas');
      $data = array(
        'nilai_id' => $id,
        ...
        'nilai_item_empatbelas' => $item_empatbelas,
        'nilai_total' => $item_satu + $item_dua + $item_tiga + $item_empat + $item_lima + $item_enam + $item_tujuh + $item_delapan + $item_sembilan + $item_sepuluh + $item_sebelas + $item_duabelas + $item_tigabelas + $item_empatbelas
      );
      $save = $this->QempatbelasModel->update_qempatbelas($id,$data);
      if ($save>0){

$(document).ready(function () {
	...
	// start
	// ------------------------------------------------------------------------------------------
	var root = window.location.origin + '/hars/';
			...
			success: function (response) {
				...
				if (response != null){
					html += '' +
						...
						'<input type="text" class="form-control" name="gaji" id="gaji" value="'+response.jabatan_biaya+'" placeholder="Jumlah gaji" autocomplete="off" required>' +
						'</fieldset>';
	...
	$('.pasien-lihat').click(function (e) {
		e.preventDefault();
		var id = $(this).val();
		var getUrl = root + 'pasien/lihat/' + id;
		$.ajax({
			url : getUrl,
			type : 'ajax',
			dataType : 'json',
			success: function (response) {
				if (response != null){
					$('#lihat_nama').val(response.pasien_nama);
					$('#lihat_tempat').val(response.pasien_tempat_lahir);
					$('#lihat_tl').val(response.pasien_tanggal_lahir);
					$('#lihat_alamat').val(response.pasien_alamat);
					$('#lihat_nohp').val(response.pasien_nomor_hp);
					$('#lihat_norek').val(response.pasien_no_rekening);
					$('#lihat_tg').val(response.pasien_tanggal_pemeriksaan);
					$('#lihat_jabatan_pasien').val(response.jabatan_nama);
					$('#lihat_gaji_pokok').val(formatRupiah(response.jabatan_biaya,'Rp. '));
					console.log(response);
				}
			},
			error: function (response) {
		...
		$.ajax({
			...
			success: function (response) {
				if (response != null){
					...
					$('#edit_nama').val(response.pasien_nama);
					$('#edit_tempat').val(response.pasien_tempat_lahir);
					$('#edit_tl').val(response.pasien_tanggal_lahir);
					$('#edit_alamat').val(response.pasien_alamat);
					$('#edit_nohp').val(response.pasien_nomor_hp);
					$('#edit_norek').val(response.pasien_no_rekening);
					$('#edit_tg').val(response.pasien_tanggal_pemeriksaan);
					$('#edit_jabatan_pasien').val(response.gaji_jabatan);
	...
	$('.pasien-hapus').click(function () {
		var id = $(this).val();
		var html = '' +
			'<a href="'+root+'pasien/hapus/'+id+'" class="btn btn-danger btn-bg-gradient-x-red-pink">Hapus</a>';
		$('#hapuspasien').html(html);
	});
	...
	var options = {
		...
		list: {
			...
			onSelectItemEvent: function() {
				var value = $("#nama_pasien").getSelectedItemData().pasien_id;

				$("#id_pasien").val(value).trigger("change");
			}
		}
	};

	$('#nama_pasien').easyAutocomplete(options);
	...
	$('.gaji-lihat').click(function (e) {
		...
		$.ajax({
			...
			success: function (response) {
				if (response != null){
					$('#gaji_lihat_nama').val(response.pasien_nama);
					$('#gaji_lihat_jabatan').val(response.jabatan_nama);
					$('#gaji_lihat_tg').val(response.pasien_tanggal_pemeriksaan);
	...
	$('.gaji-slip').click(function (e) {
		...
		$.ajax({
			...
			success: function (response) {
				if (response != null){
					$('.slip-nama').html(response.pasien_nama);
					$('#slip-jabatan').html(response.jabatan_nama);
					$('#slip-nohp').html(response.pasien_nomor_hp);
